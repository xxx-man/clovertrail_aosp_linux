...
init_variables() {
    ...
    if [ -z "${TARGET_TOOLS_PREFIX}" ]; then
        ...
        TARGET_TOOLS_PREFIX="$TOP/prebuilts/gcc/${_host_os}-x86/x86/i686-linux-android-4.6/bin/i686-linux-android-"
	...
	export PATH="$TOP/prebuilts/gcc/${_host_os}-x86/x86/i686-linux-android-4.6/bin$PATH"
...
#	make clean
    ...
    if [ "$EXTERNAL_MODULE_DIRECTORY" ]; then
        ...
        run_depmod
        exit_on_error $?
        cp -v ramdisk.img out/target/product/ramdisk.img
        pushd out/target/product/
        rm -rf ramdisk; mkdir ramdisk
        cd ramdisk
        gzip -dc ../ramdisk.img | cpio -i
        rm -rf lib
        cp -av ../root/lib/ .
        cp -av ../../../../prebuilt/* lib/modules/
        strip -g -x lib/modules/*.ko
        cd ..
        ../../../mkbootfs ramdisk | ../../../minigzip > ramdisk.img
        popd
        cp -v boot_cmdline out/target/product/boot_cmdline
	cp -v bootstub out/target/product/bootstub
